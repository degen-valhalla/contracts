...
import { IUniswapV2Router02, IUniswapV2Factory, IWETH9 } from '../../typechain-types'
...
export class UniswapV2Deployer {
  static async deploy(actor: Signer) {
  ...
  static async deployWithWETH(actor: Signer, weth9: any) {
    const deployer = new UniswapV2Deployer(actor)
  ...
  async deployWETH9() {
    return (await this.deployContract(artifacts.WETH9.abi, artifacts.WETH9.bytecode, [], this.deployer)) as IWETH9
  }
  ...
  private async deployContract(abi: any, bytecode: string, deployParams: Array<any>, actor: Signer) {
